...
uint32_t HeightHandler::get_height(uint32_t x, uint32_t y, uint32_t X, uint32_t Y) const {
...
Answer GreedySolver2::solve(TimePoint end_time) {
    ...
    while (!order.empty()) {
        ...
        uint32_t best_h = height_handler.get_height(best_x, best_y, best_x + best_length - 1,
                                                    best_y + best_width - 1);
        Position pos = {
                box.sku,
                best_x,
