...
HeightHandlerQuadtree::HeightHandlerQuadtree(uint32_t length, uint32_t width)
...
void HeightHandlerQuadtree::subdivide(Node* node) {
...
static bool intersects(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2,
                       uint32_t X1, uint32_t Y1, uint32_t X2, uint32_t Y2) {
    return x1 <= X2 && x2 >= X1 && y1 <= Y2 && y2 >= Y1;
}

// Проверка что rect1 полностью внутри rect2
static bool contains(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2,
                     uint32_t X1, uint32_t Y1, uint32_t X2, uint32_t Y2) {
    return X1 <= x1 && x2 <= X2 && Y1 <= y1 && y2 <= Y2;
}

void HeightHandlerQuadtree::update_node(Node* node, uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint32_t h) {
    if (!node) return;
    
    // Нет пересечения
    if (!intersects(x1, y1, x2, y2, node->x, node->y, node->X, node->Y)) {
        return;
    }
    
    // Узел полностью внутри области обновления
    if (contains(node->x, node->y, node->X, node->Y, x1, y1, x2, y2)) {
...
uint32_t HeightHandlerQuadtree::query_node(const Node* node, uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2) const {
    if (!node) return 0;
...
uint64_t HeightHandlerQuadtree::calc_area_at_max(const Node* node, uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint32_t target_h) const {
    if (!node) return 0;
...
uint32_t HeightHandlerQuadtree::get(uint32_t x, uint32_t y, uint32_t X, uint32_t Y) const {
    return query_node(root.get(), x, y, X, Y);
}

void HeightHandlerQuadtree::add_rect(uint32_t x, uint32_t y, uint32_t X, uint32_t Y, uint32_t h) {
...
uint64_t HeightHandlerQuadtree::get_area_at_max_height(uint32_t x, uint32_t y, uint32_t X, uint32_t Y) const {
    uint32_t max_h = query_node(root.get(), x, y, X, Y);
    return calc_area_at_max(root.get(), x, y, X, Y, max_h);
}

std::vector<std::pair<uint32_t, uint32_t>> HeightHandlerQuadtree::get_dots(
