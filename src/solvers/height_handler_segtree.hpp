...
inline uint32_t st_next_power_of_two(uint32_t n) {
    uint32_t p = 1;
    while (p < n) p *= 2;
    return p;
}
...
HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::HeightHandlerSegTreeT(uint32_t length, uint32_t width)
    ...
    tree_size_x = st_next_power_of_two(grid_width);
    tree_size_y = st_next_power_of_two(grid_height);
...
template<uint32_t CELL_SIZE_X, uint32_t CELL_SIZE_Y>
void HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::y_push(uint32_t x_idx, uint32_t vy, uint32_t ly, uint32_t ry) {
...
template<uint32_t CELL_SIZE_X, uint32_t CELL_SIZE_Y>
void HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::y_update(uint32_t x_idx, uint32_t vy, uint32_t ly, uint32_t ry,
...
// Запрос Y-дерева
template<uint32_t CELL_SIZE_X, uint32_t CELL_SIZE_Y>
uint32_t HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::y_query(uint32_t x_idx, uint32_t vy, uint32_t ly, uint32_t ry,
...
template<uint32_t CELL_SIZE_X, uint32_t CELL_SIZE_Y>
void HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::x_update_simple(uint32_t x1, uint32_t x2, uint32_t y1, uint32_t y2, uint32_t val) {
...
template<uint32_t CELL_SIZE_X, uint32_t CELL_SIZE_Y>
uint32_t HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::x_query_simple(uint32_t x1, uint32_t x2, uint32_t y1, uint32_t y2) {
    uint32_t result = 0;
    for (uint32_t x = x1; x <= x2; ++x) {
        if (x < tree_size_x) {
...
uint32_t HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::get_h(uint32_t x, uint32_t y, uint32_t X, uint32_t Y) const {
    ...
    return const_cast<HeightHandlerSegTreeT*>(this)->x_query_simple(cx1, cx2, cy1, cy2);
}
...
uint64_t HeightHandlerSegTreeT<CELL_SIZE_X, CELL_SIZE_Y>::get_area(uint32_t x, uint32_t y, uint32_t X, uint32_t Y) const {
    ...
    for (uint32_t cx = cx1; cx <= cx2; ++cx) {
        for (uint32_t cy = cy1; cy <= cy2; ++cy) {
            if (cx < tree_size_x) {
                uint32_t cell_h = const_cast<HeightHandlerSegTreeT*>(this)->y_query(cx, 1, 0, tree_size_y - 1, cy, cy);
                if (cell_h == max_h) {
