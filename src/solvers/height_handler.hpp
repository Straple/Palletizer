...
//
// ВАЖНО: Эта структура данных основана на свойстве задачи палетизации:
// прямоугольники добавляются "сверху", т.е. новый прямоугольник всегда
// имеет высоту строго больше, чем все прямоугольники в той же области.
// Поэтому два прямоугольника на одной высоте НИКОГДА не перекрываются.
// Это гарантирует корректность get_area() без сложной логики разбиения.
//
// Сложность операций:
// - get_h():    O(n), где n — количество добавленных прямоугольников
// - get_area(): O(n)
// - add_rect(): O(1)
class HeightHandler {
