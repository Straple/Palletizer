...
\subsection{Перевязка слоёв (Interlocking)}

Паллета не должна рассыпаться, что достигается минимизацией количества ``совпадающих'' щелей в двух соседних слоях. Качество перевязки описывается функцией:

$$G = K_{int} \times (L_{SumPer} - L_{Sum})$$

где:
\begin{itemize}
    \item $K_{int}$ --- весовой коэффициент
    \item $L_{SumPer}$ --- суммарный периметр оснований всех коробок
    \item $L_{Sum}$ --- суммарная длина совпадающих кусков периметров коробок, стоящих друг на друге
\end{itemize}

Нормализованная метрика:
$$InterlockingRatio = \frac{L_{Sum}}{L_{SumPer}}$$

Чем меньше $InterlockingRatio$, тем лучше перевязка (рёбра верхних коробок не совпадают с рёбрами нижних).

\subsection{Центр тяжести}

Паллета имеет достаточную устойчивость, если центр масс находится не выше некоторого заданного расстояния $H$ от уровня земли:

$$CoM_z = \frac{\sum_{i=1}^{K} m_i \cdot z_i^{center}}{\sum_{i=1}^{K} m_i}$$

где $m_i$ --- масса коробки, $z_i^{center}$ --- высота центра коробки.

Низкий центр тяжести обеспечивает устойчивость паллеты при транспортировке.
...
\begin{verbatim}
stability_score, interlocking_ratio, center_of_mass_z
\end{verbatim}
